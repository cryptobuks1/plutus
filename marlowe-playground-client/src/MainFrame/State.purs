...
handleAction Init = do
  ...
  void
    $ runMaybeT do
        ...
        let
          metadata = either mempty identity $ runExcept (decodeJSON metadataJSON)
...
handleAction (NewProjectAction (NewProject.CreateProject lang)) = do
  modify_
    ( set _projectName "New Project"
        ...
        <<< set _contractMetadata mempty
    )
  liftEffect do
    SessionStorage.setItem gistIdLocalStorageKey mempty
    SessionStorage.setItem metadataLocalStorageKey (encodeJSON (mempty :: MetaData))
...
handleAction (DemosAction action@(Demos.LoadDemo lang (Demos.Demo key))) = do
  liftEffect $ SessionStorage.setItem metadataLocalStorageKey (encodeJSON (mempty :: MetaData)) -- ToDo: Load metadata for example
  case lang of
    Haskell ->
  ...
  modify_
    ( set _showModal Nothing
        <<< set _workflow (Just lang)
        <<< set _hasUnsavedChanges false
        <<< set _contractMetadata mempty -- ToDo: Load metadata for example
    )
  selectView $ selectLanguageView lang
